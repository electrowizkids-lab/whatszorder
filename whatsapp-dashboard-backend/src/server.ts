...
// ─────────────────────────────────────────────────────────────
// PRODUCTS — the merchant's self-serve catalog (feeds the bot)
// ─────────────────────────────────────────────────────────────
...
app.patch('/api/products/:id', async (req: AuthedRequest, res: Response) => {
  ...
  vals.push(req.params.id, req.merchantId);
  try {
    const [result]: any = await pool.query(
      `UPDATE products SET ${sets.join(', ')} WHERE id = ? AND merchant_id = ?`, vals
    );
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Not found' });
...
app.delete('/api/products/:id', async (req: AuthedRequest, res: Response) => {
  try {
    const [result]: any = await pool.query(
      'DELETE FROM products WHERE id = ? AND merchant_id = ?', [req.params.id, req.merchantId]
    );
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Not found' });
    res.json({ success: true });
  } catch (e) {
...
app.delete('/api/payment-account', async (req: AuthedRequest, res: Response) => {
  try {
    await pool.query(
    ...
    console.error('Error removing payment account:', e);
